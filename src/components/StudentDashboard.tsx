...
import { isSupabaseConfigured, SUPABASE_SQL_藍圖, syncLocalStorageToSupabase } from "../lib/supabase";
  ...
  return (
    <div id="student-dashboard" className="min-h-screen bg-[#F8F9FA] pt-24 sm:pt-28 pb-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8 text-left">
        ...
        {/* INTERACTIVE SUPABASE INTEGRATION CONSOLE */}
        <div className="bg-white border border-gray-150 rounded-3xl p-6 sm:p-8 space-y-6 shadow-sm">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center gap-4 text-left">
              <div className={`p-3 rounded-2xl ${isSupabaseConfigured ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-600"} shrink-0`}>
                <Database className="w-8 h-8 animate-pulse" />
              </div>
              <div className="space-y-1">
                <div className="flex flex-wrap items-center gap-2">
                  <h3 className="font-display text-lg font-black text-[#08142B] tracking-tight">
                    Supabase Database Engine Integration
                  </h3>
                  {isSupabaseConfigured ? (
                    <span className="inline-flex items-center gap-1 text-[10px] font-mono font-bold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-100">
                      CONNECTED & LIVE
                    </span>
                  ) : (
                    <span className="inline-flex items-center gap-1 text-[10px] font-mono font-bold text-amber-700 bg-amber-50 px-2 py-0.5 rounded-full border border-amber-100">
                      DEMO LOCAL MODE
                    </span>
                  )}
                </div>
                <p className="text-xs text-slate-500 max-w-xl">
                  {isSupabaseConfigured
                    ? "Congratulations! Your academy application is permanently bound to your direct PostgreSQL database tables on Supabase."
                    : "The academy is currently operating on client-side state. Provide your VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY to turn it into a full-stack system."}
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-2 shrink-0">
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-bold text-[#08142B] bg-gray-50 hover:bg-gray-100 rounded-xl border border-gray-200 cursor-pointer transition-all"
              >
                <Table className="w-3.5 h-3.5" />
                <span>{showSqlBlueprint ? "Close Schema SQL" : "Show SQL Code"}</span>
              </button>
              ...
              <button
                ...
                disabled={isSyncing}
                className={`inline-flex items-center gap-1.5 px-4 py-2 text-xs font-bold text-white rounded-xl transition-all shadow-sm ${
                  isSupabaseConfigured 
                    ? "bg-[#2D7FF9] hover:bg-blue-600 cursor-pointer" 
          ...
          {/* Expanded SQL Blueprints Copy Section */}
          {showSqlBlueprint && (
            <div className="bg-slate-900 rounded-2xl p-6 text-left border border-slate-800 space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="space-y-0.5">
                  <span className="text-[10px] font-mono text-slate-400 block uppercase tracking-wider font-extrabold">Supabase Setup Guide</span>
                  <p className="text-[11px] text-slate-300">
                    Paste this exact SQL inside your Supabase Dashboard SQL Editor &bull; <strong>Saves instantly!</strong>
                  </p>
                </div>
                <button
                  onClick={() => {
                    navigator.clipboard.writeText(SUPABASE_SQL_藍圖.trim());
                    setCopiedSql(true);
                    setTimeout(() => setCopiedSql(false), 2000);
                  }}
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white/10 hover:bg-white/15 text-white font-bold text-[10px] font-mono rounded-lg transition-all cursor-pointer"
                >
                  {copiedSql ? (
                    <>
              ...
              <div className="relative">
                <pre className="text-[10px] sm:text-xs font-mono text-slate-300 bg-slate-950/85 p-4 rounded-xl max-h-60 overflow-y-auto leading-relaxed border border-white/5 scrollbar-thin">
                  {SUPABASE_SQL_藍圖.trim()}
                </pre>
              </div>

              {!isSupabaseConfigured && (
                <div className="bg-blue-950/30 border border-blue-500/20 rounded-xl p-4 text-[11px] text-slate-300 leading-relaxed flex items-start gap-3">
                  <span className="text-xl">💡</span>
                  <div>
                    <strong className="text-white block mb-0.5">Activation Sequence:</strong>
                    Go to the <strong>Settings (cog icon)</strong> in AI Studio, add the <strong>VITE_SUPABASE_URL</strong> and <strong>VITE_SUPABASE_ANON_KEY</strong> values from your Supabase Project settings, and this page will immediately trigger synchronization live on the fly!
                  </div>
                </div>
              )}
            </div>
          ...
          {/* Sync success toast */}
          {syncSuccess && (
            <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-4 text-xs text-emerald-800 font-bold flex items-center gap-2.5">
              <Server className="w-4 h-4 text-emerald-600" />
              <span>Congratulations! All local student progress, qualified leads, and active registry logs are successfully synchronized securely to your Supabase PostgreSQL cloud tables.</span>
            </div>
          )}
        </div>
