...
-- 7. CREATE ADMIN ACCOUNTS TABLE (secured single admin account trigger)
...
-- Admin accounts access policies (Secured by trigger)
DROP POLICY IF EXISTS "Allow public select on admin_accounts" ON public.admin_accounts;
CREATE POLICY "Allow public select on admin_accounts" ON public.admin_accounts FOR SELECT TO PUBLIC USING (true);

DROP POLICY IF EXISTS "Allow public insert on admin_accounts" ON public.admin_accounts;
CREATE POLICY "Allow public insert on admin_accounts" ON public.admin_accounts FOR INSERT TO PUBLIC WITH CHECK (true);

DROP POLICY IF EXISTS "Allow public update on admin_accounts" ON public.admin_accounts;
CREATE POLICY "Allow public update on admin_accounts" ON public.admin_accounts FOR UPDATE TO PUBLIC USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Allow public delete on admin_accounts" ON public.admin_accounts;
CREATE POLICY "Allow public delete on admin_accounts" ON public.admin_accounts FOR DELETE TO PUBLIC USING (true);
